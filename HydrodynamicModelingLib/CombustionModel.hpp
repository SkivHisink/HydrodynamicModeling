...
public:
	...
	template<typename T>
	void initDensity(std::string density_source, array3d<T>& density)
	{
		...
		if (!init_density(density, 10, ro1, density_source))
		{
	...
	template<typename T>
	void init(array3d<T> density_tmp, int typeOfFigure_)
	{
